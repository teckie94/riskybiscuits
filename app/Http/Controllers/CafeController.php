...
use App\Exports\CafesExport;
use App\Imports\CafesImport;
    ...
    public function updatecafe(Request $request, Cafes $cafe)
        {
            ...
            $request->validate([
                'name'           => 'required',
                'student_number' => 'required|unique:records,student_number,' . $cafe->id,
                'batch_number'   => 'required',
                'email'          => 'nullable|unique:records,email,' . $cafe->id,
                'mobile_number'  => 'nullable|numeric|digits:8',
        ...
        public function restore(Request $request) {
            $record = Cafes::where('id', $request->id)
            ->withTrashed()
             ->first();
       ...
       public function uploadCafes(Request $request) {
        ...
        public function exportCafes() {
            ...
            $fileName = 'customers_' . $currentDate . '.xlsx'; // Create the new file name with the current date
